...
export class BattleRoom extends Room<Stuff> {
  ...
  onCreate(options: any) {
    ...
    this.onMessage('skinLoad', (client, message) => {
      db.get(`SELECT * FROM accounts WHERE username="${message.owner}"`, (err, row) => {
        if(row.skinned) this.broadcast('bread', { player: 2, skinned: row.skinned, skinPng: row.skinPng, skinXml: row.skinXml });
      });
      if(message.name){
        db.get(`SELECT * FROM accounts WHERE username="${message.name}"`, (err, row) => {
          if(row.skinned) this.broadcast('butter', { player: 1, skinned: row.skinned, skinPng: row.skinPng, skinXml: row.skinXml });
        });
      }
    });
  }
